...
1. **If `.logbook/` doesn't exist**, reply: `📋 No logbook in this project yet. Run /jot <note> or start a tracked task and one will be created.` and stop.
2. Read `.logbook/index.md` and count tasks per status.
3. Read `.logbook/inbox.md` and count untriaged items (lines starting with `- `).
4. For each task in `.logbook/active/`, read the file and count Plan checkboxes (`- [x]` done vs `- [ ]` total).
